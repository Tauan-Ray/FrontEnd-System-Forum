...
import { RestoreAnswerAction } from "../../actions/RestoreAction";
...
export default function RestoreAnswerDialog({ idAnswer, handleReloadQuestions }: RestoreQuestionDialogProps) {
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const handleRestore = async () => {
    setIsLoading(true);
    const res = await RestoreAnswerAction(idAnswer);
      ...
      >
        <DialogHeader className="space-y-2 text-center">
          ...
          <DialogDescription className="text-muted-foreground text-sm text-center">
            Tem certeza de que deseja restaurar essa resposta?
          </DialogDescription>
        </DialogHeader>
