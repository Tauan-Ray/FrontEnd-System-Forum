...
async function revalidateToken(
  refreshToken: string
): Promise<Pick<EncryptProps, "access_token" | "access_token_expiresIn">> {
  return await service_api
...
export async function createSession(payload: SessionPayload) {
  ...
  console.log(
    ...
    "\u{1F44B} Usuário\x1b[36m",
    payload?.email,
    "\x1b[0macabou de se conectar"
  );
  ...
  cookieStore.set(USER_TOKEN, session?.access_token, {
    ...
    expires: payload?.password_remember
      ? new Date(session.access_token_expiresIn) || expiresAt
      : undefined,
    sameSite: "lax",
    path: "/",
  });
  ...
  cookieStore.set(REFRESH_TOKEN, session?.refresh_token, {
    secure: webConfig.env === "production" ? true : false,
    expires: payload?.password_remember
      ? new Date(session.refresh_token_expiresIn) || expiresAt
      : undefined,
    sameSite: "lax",
    path: "/",
  });
...
export async function UploadUserImage(file: File) {
  ...
  const { uploadUrl } = await service_api
    .post("storage/presign")
    .then(({ data }) => ({ ...data }))
    .catch(handleApiError);

  return await service_api
    .put(uploadUrl, formData.get("image"), {
      headers: {
...
export async function resetPassword(
  value: z.infer<typeof ResetPasswordFormSchema>
